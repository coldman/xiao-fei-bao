		...
		__init : function () { 
			...
			if (!styled) {
				...
				styles = [
					...
					'.jstree-grid-separator {display: inline-block; border-left: 1px solid #DDDDDD; width:0;}',
					'.jstree-grid-header-themeroller {border: 0; padding: 1px 3px;}',
					'.jstree-grid-resizable-separator {cursor: col-resize;}',
					'.jstree-grid-separator-regular {border-color: #DDDDDD;}',
					...
					if (a.length === 1) {
						...
						for (i=1;i<cols.length;i++) {
							...
							if (s === "attr") { val = col.value && t.attr(col.value) ? t.attr(col.value) : "";
							    if (col.formatter) {val = col.formatter;
								val = val.replace(/%s/g, t.attr(col.value)).replace(/%d/g, t.attr('id'));}
